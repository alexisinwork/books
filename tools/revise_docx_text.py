...
def body_paragraphs(doc):
    """Return body paragraphs, including paragraphs inside block content controls."""
    result = []
...
def repair_georgia_embedding(doc):
    """Remove the source's mislabeled CJK Georgia embedding in reader output only."""
    changed = False
    for rel in doc.part.rels.values():
        ...
        for font in root.findall(qn("w:font")):
            ...
            family = font.find(qn("w:family"))
            pitch = font.find(qn("w:pitch"))

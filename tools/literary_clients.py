...
Claude streaming envelopes: https://code.claude.com/docs/en/headless
"""
...
def _parse(client: str, stdout: str, response_path: Path, agy_log: str) -> dict:
    ...
    info = {
        "response": "", "events": [], "errors": [], "observed_models": [], "parse_error": None,
        "usage_models": [], "auxiliary_models": [], "assistant_models": [],
        "model_evidence": None, "assistant_response_matches_result": None,
    }
    try:
        if client == "claude":
            try:
                decoded = json.loads(stdout)
                # Preserve old immutable JSON receipts and offline fixtures.
    ...
    for event in events:
        ...
        if client == "claude" and event.get("type") == "assistant" and event.get("error"):
            info["errors"].append(event)
    models = []
    if client == "codex":
        ...
        if client == "agy":
            ...
            if len(results) != 1 or events[-1] is not data:
                info["parse_error"] = "Claude requires one terminal result envelope"
            usage = data.get("modelUsage", {})
            if isinstance(usage, dict):
                info["usage_models"] = sorted(key for key, value in usage.items() if isinstance(value, dict))
            # modelUsage is an invocation-wide cost/usage breakdown. Auxiliary
            # requests (for example Haiku classifiers) are not evidence that
            # Haiku wrote the final response. Direct complete SDK assistant
            # envelopes can bind response identity without guessing from tokens.
            assistants = [event for event in events if event.get("type") == "assistant"]
            assistant_models, texts = [], []
            identities_complete = True
            for event in assistants:
                message = event.get("message")
                if not isinstance(message, dict):
                    info["parse_error"] = "Claude assistant envelope lacks a message object"
                    identities_complete = False
                    continue
                model = message.get("model")
                if message.get("role") == "assistant" and isinstance(model, str) and model.strip():
                    assistant_models.append(model)
                else:
                    identities_complete = False
                content = message.get("content")
                if not isinstance(content, list) or any(not isinstance(block, dict) for block in content):
                    info["parse_error"] = "Claude assistant envelope lacks complete content blocks"
                    continue
                text_blocks = [block for block in content if block.get("type") == "text"]
                if any(not isinstance(block.get("text"), str) for block in text_blocks):
                    info["parse_error"] = "Claude assistant text block is malformed"
                    continue
                if text_blocks:
                    texts.append("".join(block["text"] for block in text_blocks))
            info["assistant_models"] = sorted(set(assistant_models))
            if assistants:
                matches = bool(texts) and isinstance(response, str) and texts[-1] == response
                info["assistant_response_matches_result"] = matches
                if not matches:
                    info["parse_error"] = "Claude terminal result differs from the last complete assistant text"
            direct = (assistants and identities_complete and info["assistant_response_matches_result"]
                      and not info["parse_error"] and not info["errors"])
            if direct:
                models.extend(info["assistant_models"])
                info["model_evidence"] = "assistant_message"
                info["auxiliary_models"] = sorted(set(info["usage_models"]) - set(models))
                if len(models) != 1:
                    info["parse_error"] = "Claude used multiple assistant models in one requested response"
            else:
                # No response-bound evidence: retain the conservative list. Do
                # not select its largest token count, last key, or requested ID.
                models.extend(sorted(set(info["usage_models"]) | set(assistant_models)))
                info["model_evidence"] = "model_usage" if not assistant_models else "unbound_assistant_metadata"
...
def execute(job: dict, attempt_dir: Path, timeout_seconds: float) -> dict:
    ...
    if error:
        ...
        if job["client"] == "agy" and len(stdin) > AGY_MAX_INPUT_BYTES:
            ...
            attested = len(models) == 1 and not (job["client"] == "claude" and (info["parse_error"] or info["errors"]))
            record.update(observed_models=models, observed_model=models[0] if len(models) == 1 else None,
                          model_attested=attested, observed_tool_events=_tools_observed(info["events"]))
            for key in ("usage_models", "auxiliary_models", "assistant_models", "model_evidence", "assistant_response_matches_result"):
                record[key] = info[key]

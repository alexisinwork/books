...
def run(args) -> None:
    ...
    write(args.out / "prompt.txt", payload)
    log_path = args.out / ("agy.log" if args.client == "agy" else "client.log")
    if args.client == "agy":
        command = ["agy", "--model", args.model, "--effort", args.effort,
                   "--output-format", "json", "--mode", "plan", "--sandbox",
                   "--disable-slash-commands", "--print-timeout", args.timeout,
                   "--log-file", str(log_path), "--print=" + payload]
        stdin = None
    else:
    ...
    receipt = {
        ...
        "isolation": "Fresh external process; all inputs embedded; sandbox/safe mode; no permitted model tools or MCP.",
        "status": "running"
    }

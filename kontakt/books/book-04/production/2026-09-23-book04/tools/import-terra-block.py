...
expected_count=max(s['chapter'] for s in cards['scenes'])
assert a.last<=expected_count
...
outputs={
    ...
    'progress.json':{'phase':'terra_full','model_requested':'gpt-5.6-terra','chapters_done':a.last,'chapters_expected':expected_count,
                     'words':sum(c['words'] for c in chapters),'status':'all_chapters_written_pending_assembly' if a.last==expected_count else 'in_progress',

...
expected_count = max(s['chapter'] for s in read(run / 'structure-v1/chapter-scene-cards.json')['scenes'])
assert assembly['through_chapter'] == expected_count
assert b'\r' not in raw
text = raw.decode('utf-8')
assert '\ufffd' not in text and '????' not in text
assert len(text.rstrip('\n').split('\n\n')) == assembly['paragraphs']
assert re.findall(r'(?m)^# Розділ (\d+)$', text) == [str(n) for n in range(1, expected_count + 1)]
files = sorted((phase / 'chapters').glob('chapter-*.md'))
assert [f.name for f in files] == [f'chapter-{n:02d}.md' for n in range(1, expected_count + 1)]
...
assert len(state['chapters']) == expected_count
scene_ids = []
quotes = 0
...
report = {
    ...
    'chapters': expected_count, 'scenes': len(scene_ids), 'paragraphs': assembly['paragraphs'],

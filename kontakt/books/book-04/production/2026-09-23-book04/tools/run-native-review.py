"""Fresh native Codex reviewer, exact requested model, immutable raw trace."""
...
a=p.parse_args()
assert not a.out.exists(), 'Use a new run directory'
a.out.mkdir(parents=True)
payload=a.prompt.read_bytes()
(a.out/'prompt.md').write_bytes(payload)
report=(a.out/'REPORT.md').resolve()
command=['codex','exec','--ignore-user-config','--skip-git-repo-check','--ephemeral',
         '--sandbox','read-only','--model',a.model,'--json','--output-last-message',str(report),'-']
record={'model_requested':a.model,'client':'codex exec','actual_backend':'not independently attested',
        'context':'fresh; isolated temporary cwd; no project files unless included verbatim in prompt',
        'command':command,'prompt_sha256':hashlib.sha256(payload).hexdigest(),'status':'running'}
def save():
    (a.out/'invocation.json').write_bytes((json.dumps(record,ensure_ascii=False,indent=2)+'\n').encode())
save()
started=time.monotonic()
with tempfile.TemporaryDirectory(prefix='kontakt-native-review-') as cwd:
...
record.update(exit_code=code,seconds=round(time.monotonic()-started,2),
    status='returned_pending_manual_validation' if code==0 and report.exists() else 'unavailable_or_incomplete',
    ...
    limitation='Transport/model request does not establish backend identity or actual complete reading; inspect report and trace.')
save()

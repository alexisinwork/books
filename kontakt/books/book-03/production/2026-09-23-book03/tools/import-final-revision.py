...
root=Path(__file__).resolve().parents[6];run=Path(__file__).resolve().parent.parent;old=run/'terra-full-v1'
...
texts={n:(old/f'chapters/chapter-{n:02d}.md').read_text(encoding='utf-8') for n in range(1,33)}
...
for n,text in texts.items():
    ...
    diff.extend(difflib.unified_diff(initial[n].splitlines(True),text.splitlines(True),fromfile='terra-full-v1/'+name,tofile=a.out.name+'/'+name))

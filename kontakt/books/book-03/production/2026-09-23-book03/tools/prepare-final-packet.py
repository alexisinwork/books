...
def add(path,body=None):
 ...
 if body is None and path.suffix=='.json':
  parsed=json.loads(supplied);supplied=json.dumps(parsed,ensure_ascii=False,separators=(',',':'));assert json.loads(supplied)==parsed;body=supplied
...
if a.mode=='revision':
 ...
 add(a.phase/'changes.json');add(a.phase/'metadata-plan-accounting.json');add(a.phase/'patch-verification.json' if (a.phase/'patch-verification.json').exists() else run/'final-patch-verification.json');add(run/'terra-final-corrigendum-validation.json');add(source,'\n\n'.join(f'[P{i:05d}] '+b for i,b in enumerate(blocks,1)))
...
Return a complete Ukrainian REPORT with exact target SHA, actual per-chapter paragraph reading ranges, unread/compacted ranges and limits, architecture and literary conclusions with anchors, all-patch and metadata/dependency accounting, remaining actual findings with EXACT short final quotes and paragraph IDs, severity/confidence/minimal compatible direction for Terra, and working-volume readiness. Where changes.json includes residual B03-FINAL-01, verify that extra patch as well: it names the narrator as sender after Osya put away his device in CH29. This is another complete reading of the current source, not acceptance of an earlier report. Pending completion flags are operational tasks for root after this report; do not treat them as prose defects or claim those file writes yourself. If none found, say none within this reading rather than flawless. No invented dictionary/audio/human/Word tests, backend identity attestation or author canonical approval. Finish the entire reading now.
'''

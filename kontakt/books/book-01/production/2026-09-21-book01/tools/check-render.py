...
stats=[]
sheet=fitz.open(); composite=None; contacts=[]
for i,p in enumerate(doc):
    if i%6==0:
        remaining=min(6,len(doc)-i)
        composite=sheet.new_page(width=620,height=((remaining+1)//2)*445)
    pix=p.get_pixmap(matrix=fitz.Matrix(1,1),alpha=False)
    pix.save(out/f'page-{i+1:02d}.png')
    x=(i%2)*310; y=((i%6)//2)*445
...
for i,composite in enumerate(sheet):
    name='contact.png' if len(sheet)==1 else f'contact-{i+1:03d}.png'
    composite.get_pixmap(matrix=fitz.Matrix(1,1),alpha=False).save(out/name)
    contacts.append({'path':name,'first_page':i*6+1,'last_page':min((i+1)*6,len(doc))})
report={'renderer':'docx-preview/Chromium; not native Word','pdf_sha256':hashlib.sha256(pdf.read_bytes()).hexdigest(),'pages':len(doc),'text_matches_docx_ignoring_layout_whitespace':norm(expected)==norm(extracted),'page_checks':stats,'contact_sheets':contacts,'visual_review':'pending'}

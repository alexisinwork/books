...
doc.core_properties.title = 'Контакт — глава 1 — перша чернетка'
...
dest = reader / 'Контакт — глава 1 — чернетка.docx'
doc.save(dest)
actual = [p.text for p in Document(dest).paragraphs]
assert actual == expected
(base/'anchors.json').write_bytes((json.dumps({'source':'chapter-01.md','source_sha256':sha,'paragraphs':anchors},ensure_ascii=False,indent=2)+'\n').encode('utf-8'))

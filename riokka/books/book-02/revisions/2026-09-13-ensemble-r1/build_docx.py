...
OUTPUT = REV / "Риокка — Книга 2 — Прочие убытки — ансамблевая редакция.docx"
...
def main() -> None:
    ...
    props.subject = "Ансамблевая редакция для авторской вычитки"
    props.author = ""
    ...
    page_breaks = sum(len(p._element.xpath('.//w:br[@w:type="page"]')) for p in check.paragraphs)
    if page_breaks != 51:

...
NAME = 'Риокка — Книга 3 — Холм Розмари — редакция для вычитки'
...
def build():
    ...
    profile = Path(tempfile.mkdtemp(prefix='riokka-book3-writer-'))
    ...
    selections = sorted({1, 2, len(pages), *(chapter_pages[n] for n in [3, 14, 29, 46, 49, 50, 51, 52, 53, 54, 55, 57, 58, 59, 60])})
    for n in selections:
    ...
    write(render / 'verification.json', {'source_sha256': sha(source), 'docx_sha256': sha(target), 'pdf_sha256': sha(pdf),
        'docx_paragraphs': len(expected), 'chapter_count': 60, 'page_count': len(pages), 'docx_text_exact': True,
    ...
    write(R / 'release.json', {'source': 'revised.md', 'source_sha256': sha(source), 'status': 'editorial_revision_for_author_review',
        'docx': target.name, 'docx_sha256': sha(target), 'pdf': str(pdf.relative_to(R)), 'pdf_sha256': sha(pdf),
        'chapters': 60, 'pages': len(pages), 'reader_text': 'derived/reader.txt', 'verification': 'audit/render/verification.json',

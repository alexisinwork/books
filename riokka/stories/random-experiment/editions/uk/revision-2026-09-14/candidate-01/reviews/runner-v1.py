...
def run(args):
    ...
    if args.role in {"gemini_flash", "gemini_pro"} and args.client != "agy":
    ...
    record = {"role": args.role, "client": args.client, "requested_model": args.model,
              "target_sha256": digest, "source_sha256": sha(args.source) if args.source else None,
    ...
    if args.client == "agy":
        command = ["agy", "--new-project", "--model", args.model, "--mode", "plan", "--sandbox",
                   "--disable-slash-commands", "--output-format", "json", "--print-timeout", "180s", "--print", payload]
    else:
    ...
    with tempfile.TemporaryDirectory(prefix="book-independent-reader-") as folder:
        try:
            response = subprocess.run(command, cwd=folder, capture_output=True, text=True, timeout=240)
    ...
    valid = code == 0 and len(report.strip()) >= 200 and digest in report and not (isinstance(data, dict) and data.get("is_error"))

...
def main():
    ...
    for src in (ROOT/'tools/language_qa.py',ROOT/'tools/uk_naturalness.py', language_qa.SNAPSHOT_PATH,
                BASE/'tools/revise_docx_text.py',BASE/'tools/render_docx.py',Path(__file__)):
    ...
    for item in json.loads((BASE/'config.json').read_text())['stories']:
        ...
        complete_manifest = folder/'complete-projection-manifest.json'
        suffix = '.complete' if complete_manifest.exists() else ''
        expected_units = item['expected_units']
        if suffix:
            complete = json.loads(complete_manifest.read_text())
            expected_units = complete['complete_units']
            for entry in complete['files']:
                assert sha(folder/entry['path']) == entry['sha256']
        ru_path = folder/f'SOURCE/ru{suffix}.txt'
        ru = edit.read_projection(ru_path)
        desktop = edit.read_projection(folder/f'SOURCE/desktop-uk{suffix}.txt')
        assert len(uk)==len(ru)==len(desktop)==expected_units
        ...
        after = Document(docx)
        assert len(edit.body_paragraphs(before))==len(edit.body_paragraphs(after))
        for p,q in zip(edit.body_paragraphs(before),edit.body_paragraphs(after)):
            assert (p._p.pPr.xml if p._p.pPr is not None else None)==(q._p.pPr.xml if q._p.pPr is not None else None)
        for paragraph in edit.body_paragraphs(after):
        ...
        save(meta/'alignment.json',{'source_sha256':sha(ru_path),'target_sha256':sha(final),
                                    'mapping':'1:1 complete body paragraph mapping; length is only a diagnostic signal','items':alignment})
        changes={}
        prepared_path = folder/'SOURCE/prepared-uk.complete.txt' if suffix else folder/source_manifest['chosen_editorial_base']
        for label,old in [('desktop',desktop),('prepared',edit.read_projection(prepared_path))]:
        ...
        evidence={'story':item['id'],'title':item['title'],'paragraphs':len(uk),'docx':str(docx.relative_to(out)),
                  ...
                  'complete_projection_manifest':str(complete_manifest.relative_to(BASE)) if suffix else None,
                  'formatting_note':'Malformed CJK Georgia font embedding removed in reader copy; Georgia family retained.' if item['id']=='random-experiment' else None}
